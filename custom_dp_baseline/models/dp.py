...
class DiffusionPolicy(nn.Module):
    ...
    def __init__(self, args):
        ...
        # N-agnostic: camera features are mean-pooled in _build_conds, so the
        # global cond dim is feature_dim per camera count is independent of
        # num_cameras -> the same checkpoint serves any N (e.g. 4-cam train,
        # 1-cam eval).
        image_feat_dim_total = self.rgb_encoder.feature_dim * n_obs_steps
    ...
    def _build_conds(self, data_dict: dict, device: torch.device, dtype: torch.dtype):
        ...
        # N-agnostic camera conditioning: encode each view with the shared
        # per-camera encoder, then mean-pool over cameras -> (B, F) for any N.
        # The same checkpoint therefore accepts N=4 (training) and N=1 (eval).
        n = images.shape[1]
        if n != self.num_cameras:
            print(f"  [dp] camera-count mismatch: model built for {self.num_cameras}, got {n}; using mean-pool over {n} views")
        img_bsnchw = einops.rearrange(images, "b n c h w -> (b n) c h w")
        img_feats = self.rgb_encoder(img_bsnchw)  # ((B*N), F)
        img_feats = einops.rearrange(img_feats, "(b n) f -> b n f", b=B, n=n)
        global_cond = img_feats.mean(dim=1)  # (B, F)
