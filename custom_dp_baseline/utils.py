...
def fix_scene_setup(env):
    ...
    The fork's envs re-sample two visual planes at every env.reset():
    the table-top plane (plane_sampler / table_plane_sampler) and the
    floor/tile plane (floor_plane_sampler). The robot base and the table
    body are already static. Pinning these samplers in place fixes the
    background while object placement (ObjectSampler) and the robot's
    end-effector pose (initialization_noise="default") stay random.

    Two mechanisms, both applied only when such samplers exist:
    ...
    Classic robosuite tasks (e.g. Lift) have no such samplers (static setup),
    so this is a no-op there: the env keeps every origin behavior. Call AFTER
    env creation; every subsequent env.reset() is then fixed-setup.
    """
    pinned = []
...
class EpisodicDataset(Dataset):
    ...
    def __init__(self, demo_indices, norm_stats, args, camera_poses_file=None,
        ...
        if not self.args.default_cam:
            ...
            if 'camera_names' in raw:
                # RIG format: one per-demo rig per entry, e.g.
                # {"format":"rig","camera_names":[left,right,top,bottom],
                #  "poses":[[N x 4x4] per demo]}
                self.pose_format = 'rig'
                self.camera_names = list(raw['camera_names'])
                assert len(self.camera_names) == self.num_cameras, (
                    f"rig file has {len(self.camera_names)} cameras {self.camera_names}, "
                    f"but --num_side_cam={self.num_cameras}"
                )
                print(f"Loaded {len(self.camera_poses)} per-demo rigs (rig format) from {poses_path}; "
                      f"cameras={self.camera_names}; num_side_cam={self.num_cameras}")
            else:
                # Old FLAT format: poses repeated per demo, window W(i) = [m*i, m*i+n)
                self.pose_format = 'flat'
    ...
    def __getitem__(self, demo_idx):
        ...
        cam_images = []
        if not self.args.default_cam:
            if self.pose_format == 'rig':
                # One fixed rig per demo, indexed by the dataset-local demo index
                try:
                    pose_set = self.camera_poses[demo_idx]
                except IndexError:
                    raise IndexError(
                        f"rig pose file has only {len(self.camera_poses)} rigs but demo index {demo_idx} "
                        f"was requested — regenerate the rig JSON with at least "
                        f"--train_demos/--test_demos >= num_episodes+10"
                    ) from None
            else:
        ...
        for cam_pose_raw in pose_set:
            ...
            if self.use_plucker and not self.args.default_cam:
                ...
                # Pure RGB baseline (no Plucker): keep C=3
                img_chw = rgb_tensor
        ...
        # Camera extrinsics tokens: one 4x4 per camera in the pose set (N entries)
        if self.args.use_cam_pose and not self.args.default_cam:
            cam_extrinsics_list = []
            for i in range(len(pose_set)):
                if pose_set[i] is not None:
            ...
            cam_extrinsics = torch.zeros(len(pose_set), 4, 4, device='cuda')
