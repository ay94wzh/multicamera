...
class Evaluator:
    ...
    def __init__(self, env, norm_stats, dataset_path, args):
        ...
        if not args.default_cam:
            ...
            # 'rig' if the pose file has per-demo camera lists (camera_names
            # key); 'flat' otherwise (poses repeated per demo, old format)
            self.pose_formats = {}
            for filename in args.pose_files:
                ...
                fmt = 'rig' if 'camera_names' in raw else 'flat'
                self.pose_formats[pose_name] = fmt
                if fmt == 'rig':
                    print(f"Loaded {len(raw['poses'])} per-demo rigs (rig format) from {poses_path}; "
                          f"cameras={list(raw['camera_names'])}; key={pose_name}")
                else:
    ...
    def _compose_frame(self, per_cam_images):
        """Lay out N per-camera frames into one video frame.

        n == 1: single image. n == 2: horizontal pair. n == 4: 2x2 grid in
        camera order (left|right / top|bottom).
        """
        n = len(per_cam_images)
        if n == 1:
            return per_cam_images[0]
        if n == 2:
            return np.concatenate([per_cam_images[0], per_cam_images[1]], axis=1)
        if n == 4:
            top_row = np.concatenate([per_cam_images[0], per_cam_images[1]], axis=1)
            bottom_row = np.concatenate([per_cam_images[2], per_cam_images[3]], axis=1)
            return np.concatenate([top_row, bottom_row], axis=0)
        raise ValueError(f"Unsupported camera count for video composition: {n}")
    ...
    def evaluate(self, policy, save_path, video_prefix, pose_name, init_state=None, episode_num=0):
        ...
        if self.args.default_cam:
            ...
            if self.pose_formats[pose_name] == 'rig':
                # One fixed rig (list of N camera matrices) per episode
                try:
                    pose_set = poses_list[episode_num]
                except IndexError:
                    raise IndexError(
                        f"rig pose file {pose_name} has only {len(poses_list)} rigs but "
                        f"episode {episode_num} was requested — regenerate it with "
                        f"--test_demos/--eval_episodes >= the episode count"
                    ) from None
        ...
        while not done and step < self.max_steps:
            ...
            camera_frame = self._compose_frame(per_cam_images)
            ...
            # Camera extrinsics: one 4x4 per camera in the pose set
            if self.args.use_cam_pose and not self.args.default_cam:
                ...
                cam_extrinsics = torch.zeros(1, len(pose_set), 4, 4, device='cuda')
